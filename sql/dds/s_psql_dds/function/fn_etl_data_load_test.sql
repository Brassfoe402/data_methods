drop function if exists s_psql_dds.fn_etl_data_load_test(date, date) cascade;

create function s_psql_dds.fn_etl_data_load_test(p_start_date date, p_end_date date)
returns table (
...
begin
    ...
    raise notice 'Завершено fn_etl_data_load_test. Загружено % записей', v_inserted_rows;
    
exception when others then
    raise notice 'Ошибка в fn_etl_data_load_test: %', sqlerrm;
    return query
